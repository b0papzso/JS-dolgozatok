...
function removeFirstAndLast(string) {
    ...
    let elvalasztott = string.split(',');
    elvalasztott = elvalasztott.slice(1,string.length-1);
    elvalasztott = elvalasztott.slice(0, elvalasztott.length-1)
    if(elvalasztott.length == 0)
    {return null}
    return elvalasztott.join(' ')
}
...
function dnaStrand(dna){
    ...
    const dnaellentet = dna.split('').map(char => ellentetek[char]).join('');
    return dnaellentet;
}
...
function deEmojify(emojiString) {
    ...
    const emojik= {
      ...
      return emojiString.split("  ").map(atvaltas => {
          atvaltas = atvaltas.split(" ");
          atvaltas = atvaltas.map(s => s = emojik[s]);
          atvaltas = atvaltas.join("");
          return String.fromCharCode(atvaltas)
      }).join("")
}
